...
class VPNGUI:
    ...
    def start_vpn(self):
        ...
        if version != "QUIC" and not os.path.isfile(key_path):
            messagebox.showerror("Error", f"Missing key file: {key_path}")
            self.log_message(f"Missing key file: {key_path}")
            return

        self.toggle_button.configure(style="Toggle.On.TButton")  # Apply orange border when on
        ...
        try:
            ...
            self.toggle_button.configure(style="Toggle.TButton")  # Revert style on error
    ...
    def stop_vpn(self):
        """Stop the VPN client."""
        self.running = False
        self.toggle_button.configure(style="Toggle.TButton")  # Remove orange border when off
        self.toggle_button.config(text="On")
        self.is_vpn_on = False
